...
//    return ret;
  ...
  int get_int_data(int * x, int i, int j, int k)
  {
    i = CLAMPCL(i,0,NX-1);
    j = CLAMPCL(j,0,NY-1);
    k = CLAMPCL(k,0,NZ-1);
...
int is_blocked_cell(int *obs, int i, int j, int k)
{
    return  get_int_data(obs, i, j, k);
}
...
void pressure_solve(float *pressure, float *pressure_prev,  float *divergence, int* obs, float dt)
{
    FOR_EACH_CELL
    {
    ...
    for (int ii=0; ii < 40; ++ii)
    {
        FOR_EACH_CELL
        {
            ...
            else
            {
              ...
              if(is_blocked_cell(obs,i-1, j, k) == 1) a = pC;
              if(is_blocked_cell(obs,i+1, j, k) == 1) b = pC;
              if(is_blocked_cell(obs,i, j-1, k) == 1) c = pC;
              if(is_blocked_cell(obs,i, j+1, k) == 1) d = pC;
              if(is_blocked_cell(obs,i, j, k-1) == 1) e = pC;
              if(is_blocked_cell(obs,i, j, k+1) == 1) f = pC;
